...
use std::{sync::{Mutex, Arc, atomic::{AtomicBool, Ordering}}, thread, time::Duration};
                ...
                match clipboard.get_image() {
                    Ok(img) => {
                        ...
                        if hash != last_image_hash {
                            println!("检测到新的剪贴板图片内容，尺寸: {}x{}", img.width, img.height);
                            ...
                            if let Ok(db) = app_handle.state::<Mutex<DbManager>>().lock() {
                                // 检查是否已经存在相同内容
                                let content_exists = match db.check_clipboard_entry_exists(&img_text) {
                                    Ok(exists) => exists,
                                    Err(e) => {
