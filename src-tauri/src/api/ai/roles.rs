...
pub async fn create_ai_role(name: String, description: String) -> Result<AIRole, String> {
    let db = DbManager::init().map_err(|e| e.to_string())?;
    db.create_role(&name, &description).map_err(|e| e.to_string())
}

// 更新角色
#[tauri::command(rename_all = "snake_case")]
pub async fn update_ai_role(role_id: String, name: String, description: String) -> Result<AIRole, String> {
    let db = DbManager::init().map_err(|e| e.to_string())?;
    db.update_role(&role_id, &name, &description).map_err(|e| e.to_string())
}
