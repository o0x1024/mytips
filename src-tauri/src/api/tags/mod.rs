...
pub async fn get_all_tags(db_manager: State<'_, DbManager>) -> Result<Vec<crate::db::Tag>, String> {
    ...
    crate::db::get_all_tags(&conn).await.map_err(|e| e.to_string())
}
...
pub async fn create_tag(
    ...
    let conn = db_manager.get_conn().await.map_err(|e| e.to_string())?;
    crate::db::create_tag(&conn, &name).await.map_err(|e| e.to_string())
}
...
pub async fn delete_tag(db_manager: State<'_, DbManager>, id: String) -> Result<(), String> {
    let conn = db_manager.get_conn().await.map_err(|e| e.to_string())?;
    crate::db::delete_tag(&conn, &id).await.map_err(|e| e.to_string())
}
